...
export function generateCodeVerifier(length=64){return base64url(crypto.randomBytes(length));}
export async function generateCodeChallenge(codeVerifier){const hash=crypto.createHash("sha256").update(codeVerifier).digest();return base64url(hash);}
export function randomString(n=32){return base64url(crypto.randomBytes(n));}

export async function getDiscovery(discoveryUrl){const res=await fetch(discoveryUrl);if(!res.ok)throw new Error(`Discovery failed: ${res.status}`);return res.json();}

export function buildAuthorizeUrl({authorization_endpoint,client_id,redirect_uri,scope,state,nonce,code_challenge}){
  const params=new URLSearchParams({client_id,response_type:"code",redirect_uri,scope,state,nonce,code_challenge,code_challenge_method:"S256"});
...
export async function verifyIdToken({id_token,jwks_uri,audience,expectedNonce}){
  ...
  const { payload }=await jwtVerify(id_token,JWKS,{audience,maxTokenAge:"10m",clockTolerance:300});
